    ...
    return (
        <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
            ...
            {data.map((d, i) => {
                ...
                const y = height - ((d / maxValue) * (height - 2 * padding) + padding);
                return (
                    <circle key={i} cx={x} cy={y} r="4" fill="white" stroke="#002B5B" strokeWidth="2" />
                );
            })}
        </svg>
    );
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="mt-4 text-gray-400 font-bold text-xs uppercase tracking-widest">Đang tải dữ liệu báo cáo...</p>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Cập nhật số liệu thực tế từ dữ liệu sàn.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Người dùng', value: stats.totalUsers, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50' },
                    { label: 'Đang duyệt', value: stats.pendingProducts, icon: AlertCircle, color: 'text-amber-600', bg: 'bg-amber-50' },
                    { label: 'Đấu giá active', value: stats.activeAuctions, icon: Gavel, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                    { label: 'Tổng doanh thu', value: formatCurrency(stats.totalSales), icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' }
                ].map((s, i) => (
                    <div key={i} className="bg-white p-6 rounded-lg border border-gray-100 shadow-sm flex items-start justify-between">
                        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-lg border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">7 ngày gần nhất</span>
                    </div>
                    <div className="py-6">
                ...
                <div className="bg-white p-8 rounded-lg border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">Sản phẩm/tháng</span>
                    </div>
                    <div className="py-6">
                        <SimpleBarChart data={[45, 32, 56, 42, 68, 89, 75, 92, 110, 85, 96, 120]} />
                    </div>
                    <div className="mt-6 flex justify-between text-[10px] font-bold text-gray-400 uppercase">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {[
                                { title: 'iPhone 15 Pro Max 256GB', seller: 'Minh Tuấn', status: 'Approved', time: '5m' },
                                { title: 'Rolex Submariner Date', seller: 'LuxuryWatch', status: 'Approved', time: '12m' },
                                { title: 'MacBook M3 Pro 14 inch', seller: 'Thành Đạt', status: 'Published', time: '20m' },
                                { title: 'Túi Gucci Marmont', seller: 'Thúy Hạnh', status: 'Approved', time: '34m' }
                            ].map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-8 py-4 font-bold text-gray-800">{item.title}</td>
